...
class ExtendService : Service() {
    ...
    private fun addPanel() {
        val root = LinearLayout(this).apply {
            ...
            setBackgroundColor(Color.argb(215, 8, 8, 8))
        ...
        val btnClear = Button(this).apply {
            ...
            textSize = 11f
            backgroundTintList =
                android.content.res.ColorStateList.valueOf(Color.argb(200, 60, 60, 60))
        }
        ...
        val tbAuto = ToggleButton(this).apply {
            ...
            textSize = 10f
            backgroundTintList =
                android.content.res.ColorStateList.valueOf(Color.argb(200, 40, 40, 40))
        }
        ...
        root.setOnTouchListener { v, e ->
            when (e.action) {
                ...
                MotionEvent.ACTION_MOVE -> {
                    wlp.x = (ox - (e.rawX - sx)).toInt()
                    wlp.y = (oy + (e.rawY - sy)).toInt()
        ...
        fun status(msg: String, color: Int) {
            main.post {
                tvStatus.text = msg
        ...
        btnScan.setOnClickListener {
            ...
            doScan { msg, ok ->
                status(msg, if (ok) Color.GREEN else Color.RED)
            }
        }

        btnClear.setOnClickListener {
            overlayView?.clear()
            status("Limpo", Color.GRAY)
        }
        ...
        tbAuto.setOnCheckedChangeListener { _, on ->
            ...
            if (on) {
                status("AUTO ligado", Color.GREEN)
                scheduleAuto(tvStatus)
            } else {
                status("AUTO desligado", Color.GRAY)
            }
        }
    ...
    private fun scheduleAuto(tv: TextView) {
        ...
        main.postDelayed({
            doScan { msg, ok ->
                main.post {
                    tv.text = msg
                    tv.setTextColor(if (ok) Color.GREEN else Color.RED)
                }
            }
    ...
    private fun doScan(cb: (String, Boolean) -> Unit) {
        ...
        try {
            ...
            bg.post {
                try {
                    val result = LineDetector.detect(finalBmp)
                    val lines = result.first
                    val pockets = result.second
                    ...
                    main.post {
                        overlayView?.update(lines, pockets)
                    }

                    if (lines.isEmpty()) {
                        cb("Nenhuma linha detectada", false)
                    } else {
                        cb("${lines.size} linha(s) detectada(s)", true)
                    }
    ...
    private fun lp2(mb: Int = 0): LinearLayout.LayoutParams {
        return LinearLayout.LayoutParams(-1, -2).also {
            it.bottomMargin = mb
        }
    }
    ...
    private fun buildNotif(): Notification {
        return NotificationCompat.Builder(this, CH)
            .setContentTitle("TacExtend ativo")
            .setContentText("Mire e toque Escanear")
